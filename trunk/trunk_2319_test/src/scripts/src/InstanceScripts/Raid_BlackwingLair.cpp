...
public:
    ...
    DTcaptainAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
        ...
        spells[0].info = SpellDataStorage.LookupEntry(MARK_OF_FLAMES);
		spells[0].targettype = TARGET_ATTACKING;
		spells[0].instant = false;
		spells[0].perctrigger = 15.0f;
		spells[0].attackstoptimer = 1000; // 1sec

		spells[1].info = SpellDataStorage.LookupEntry(MARK_OF_DETONATION);
...
public:
    ...
    DTflamescaleAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		// Create basic info for spells here, and play with it later , fill always the info, targettype and if is instant or not! 
        spells[0].info = SpellDataStorage.LookupEntry(FLAME_SHOCK);
		spells[0].targettype = TARGET_ATTACKING;
		spells[0].instant = false;
...
public:
    ...
    DTwyrmkinAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		// Create basic info for spells here, and play with it later , fill always the info, targettype and if is instant or not! 
        spells[0].info = SpellDataStorage.LookupEntry(FIREBALL_VOLLEY);
...
public:
    ...
    TechnicianAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		// Create basic info for spells here, and play with it later , fill always the info, targettype and if is instant or not! 
        spells[0].info = SpellDataStorage.LookupEntry(GRANADE);
		spells[0].targettype = TARGET_ATTACKING;
		spells[0].instant = false;
...
public:
    ...
    BlackWarlockAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		spells[1].info = SpellDataStorage.LookupEntry(SHADOW_BOLT);
...
public:
    ...
    LashlayerAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		// Create basic info for spells here, and play with it later , fill always the info, targettype and if is instant or not! 
        spells[0].info = SpellDataStorage.LookupEntry(FIRE_NOVA_LASH);
		spells[0].targettype = TARGET_VARIOUS;
		spells[0].instant = false;
		spells[0].perctrigger = 15.0f;
		spells[0].attackstoptimer = 1000; // 1sec
...
public:
    ...
    FiremawAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		spells[1].info = SpellDataStorage.LookupEntry(FLAME_BUFFET);
...
public:
    ...
    EbonrocAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		spells[1].info = SpellDataStorage.LookupEntry(SHADOW_OF_EBONROC);
		spells[1].targettype = TARGET_ATTACKING;
		spells[1].instant = false;
		spells[1].perctrigger = 15.0f;
		spells[1].attackstoptimer = 1000; // 1sec
...
public:
    ...
    VaelastraszAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		// Create basic info for spells here, and play with it later , fill always the info, targettype and if is instant or not! 
        spells[0].info = SpellDataStorage.LookupEntry(ESSENCE_OF_THE_RED);
		spells[0].targettype = TARGET_VARIOUS;
		spells[0].instant = false;
		...
		spells[1].info = SpellDataStorage.LookupEntry(FLAME_BREATH);
		spells[1].targettype = TARGET_VARIOUS;
		spells[1].instant = false;
		spells[1].perctrigger = 15.0f;
		spells[1].attackstoptimer = 3000; // 1sec

		spells[2].info = SpellDataStorage.LookupEntry(BURNING_ADRENALINE);
