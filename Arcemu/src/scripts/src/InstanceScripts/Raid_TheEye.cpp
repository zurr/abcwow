    ...
    void AIUpdate()
    {
		...
		if(!ArcaneOrbTimer)
		{
			...
			if(RandomTarget)
			{
				...
				uint32 time = (uint32)((_unit->CalcDistance(orbTarget) * 1000.0f) / dbcSpell.LookupEntry(ARCANE_ORB_TRIGGER)->speed + 1000.0f);
...
public:
	...
	KaelThasAI(Creature* pCreature) : MoonScriptBossAI(pCreature)
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			Creature *pCreature = static_cast<Creature*>(ForceCreatureFind(Advisors[i].mAddition, Advisors[i].mX, Advisors[i].mY, Advisors[i].mZ, false));
			if (pCreature != NULL)
			{
		...
		if (GetPhase() == 5)
		{
			...
			else if (IsTimerFinished(mEventTimer))
			{
				for (int i = 0; i < 4; ++i)
				{
					...
					Creature *pCreature = static_cast<Creature*>(ForceCreatureFind(mAdvCoords[i].mAddition, mAdvCoords[i].mX, mAdvCoords[i].mY, mAdvCoords[i].mZ));
