...
bool ChatHandler::HandleGetPosCommand(const char* args, WorldSession *m_session)
{
	...
	SpellEntry *se = SpellDataStorage.LookupEntry(spell);
	if(se)
		BlueSystemMessage(m_session, "SpellIcon for %d is %d", se->Id, se->field114);
	return true;
}

void WordFilter::Load()
{
	QueryResult * result = WorldDatabase.Query("SELECT * FROM banned_phrases");
	if(result == NULL)
		return;
	...
	do 
	{
		strs.push_back( string(result->Fetch()[0].GetString() ));
	} while(result->NextRow());
	delete result;
...
#define BLOCKED_MESSAGE "Your chat message was blocked due to a server-side content filter. This has been recorded and will be investigated in the future. Filter: '%s', Message: '%s'"

bool WordFilter::UsesBannedKeyword(string& message, WorldSession * session)
{
	string lowercasemessage = message;
	transform(lowercasemessage.begin(), lowercasemessage.end(), lowercasemessage.begin(), towlower);

	for(vector<string>::iterator itr = blockedPhrases.begin(); itr != blockedPhrases.end(); ++itr)
	{
		if(lowercasemessage.find((*itr).c_str()) != string::npos)
		{
			session->SystemMessage(BLOCKED_MESSAGE, (*itr).c_str(), message.c_str());
			LogMessage(message, *itr, session->GetPlayer());
			return true;
		}
	}
	return false;
...
void WordFilter::LogMessage(string& message, string& blockedKeyword, Player * plr)
{
	...
	snprintf(datestring, 200, "[%-4d-%02d-%02d %02d:%02d:%02d] ",aTm->tm_year+1900,aTm->tm_mon+1,aTm->tm_mday,aTm->tm_hour,aTm->tm_min,aTm->tm_sec);
	fprintf(m_file, "%s: Player: %s (%u) Account %s from %s, Filter: '%s', Message: '%s'\n", datestring,plr->GetName(),plr->GetGUIDLow(),
		plr->GetSession()->GetAccountName().c_str(), plr->GetSession()->GetSocket() ? plr->GetSession()->GetSocket()->GetRemoteIP().c_str() : "unk",
		blockedKeyword.c_str(), message.c_str());
