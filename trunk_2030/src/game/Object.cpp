...
*/	
	...
	if ((isCritter || health <= damage) )
	{
		...
		pVictim->HandleProc(PROC_ON_DEATH,pVictim,NULL);
        ...
        if(pMapInfo && pMapInfo->type == INSTANCE_NULL && !pVictim->IsPlayer() && !pVictim->IsPet() 
            && (IsPlayer() || IsPet()))
		{
		...
		if(pVictim->IsPlayer())
		{
			...
			((Player*)pVictim)->SoulStone = ((Player*)pVictim)->SoulStoneReceiver = 0;
...
//========================================================================================== 
	...
	uint32 aproc = 0;
	uint32 vproc = PROC_ANY_DAMAGE_TAKE | PROC_STRUCK_SPELL;
	bool critical = false;
//==========================================================================================
...
//					res = res*(1.0f-2.0f*static_cast<Player*>(pVictim)->CalcRating(14));
				...
				aproc |= PROC_SPELL_CRIT;
				vproc |= PROC_STRUCK_SPELL_CRIT;
			}
		}
	}
//==========================================================================================
