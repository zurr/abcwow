...
void Unit::HandleProc(uint32 flag, Unit* victim, SpellEntry* CastingSpell,uint32 dmg,uint32 abs)
{
	...
	for( itr = m_procSpells.begin();itr != m_procSpells.end();)  // Proc Trigger Spells for Victim
	{
		...
		SpellEntry *ospinfo = SpellDataStorage.LookupEntry(origId );//no need to check if exists or not since we were not able to register this trigger if it would not exist :P
		//this requires some specific spell check,not yet implemented
		if(itr2->procFlags & flag)
		{
			uint32 spellId = itr2->spellId;
			if(itr2->procFlags & PROC_CAST_SPECIFIC_SPELL)
			{
				if(!CastingSpell)
					continue;
						...
						case 25505:
							{
								...
								SpellEntry *spellInfo = SpellDataStorage.LookupEntry(origId);
								uint32 AP_owerride=GetAP() + spellInfo->EffectBasePoints[0]+1;
								float dmg = static_cast<Player*>(this)->GetMainMeleeDamage(AP_owerride);
								SpellEntry *sp_for_the_logs = SpellDataStorage.LookupEntry(spellId);
...
void Unit::HandleProcDmgShield(uint32 flag, Unit* victim)
{
	...
	for(i = victim->m_damageShields.begin();i != victim->m_damageShields.end();)     // Deal Damage to Attacker
	{
		...
		if(	(flag & (*i2).m_flags) )
		{
			...
			{
				SpellEntry	*ability=SpellDataStorage.LookupEntry((*i2).m_spellId);
...
//--------------------------------dodge-----------------------------------------------------
	case 1:
		...
		vstate = DODGE;
		vproc |= PROC_DODGE;
		aproc |= PROC_TARGET_DODGE_OR_PARRY; 
...
//--------------------------------state proc initialization---------------------------------
			vproc |= PROC_ANY_DAMAGE_TAKE;			
			if(damage_type != RANGED)
			{
				aproc |= PROC_ATTACK_MELEE;
				vproc |= PROC_STRUCK_MELEE;
			}
			else
			{
				aproc |= PROC_ATTACK_RANGED;
				vproc |= PROC_STRUCK_RANGED;
				if(ability && ability->Id==3018 && IsPlayer() && getClass()==HUNTER)
					aproc |= PROC_ATTACK_AUTO_SHOT;
			}
...
//						dmg.full_damage=float2int32(float(dmg.full_damage)*(1.0f-2.0f*static_cast<Player*>(pVictim)->CalcRating(14)));
					...
					vproc |= PROC_STRUCK_MELEE_CRIT;
					aproc |= PROC_ATTACK_MELEE_CRIT;
...
//==========================================================================================
	if(realdamage)
	{
		...
		//pVictim->HandleProcDmgShield(PROC_STRUCK_MELEE,this);
		HandleProcDmgShield(PROC_STRUCK_MELEE,pVictim);
...
void Unit::EventSummonPetExpire()
{
	if(summonPet)
	{
		...
		{
			SpellEntry *spInfo = SpellDataStorage.LookupEntry(13259);
			if(!spInfo)
				return;
...
{
	SpellEntry * ent = SpellDataStorage.LookupEntry(SpellID);
	if(ent == 0) return;
					...
					case 33151:
						{
						    ...
						    SpellEntry *spi = SpellDataStorage.LookupEntry(skip);
