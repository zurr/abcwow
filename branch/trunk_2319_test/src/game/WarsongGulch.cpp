...
void WarsongGulch::HookOnAreaTrigger(Player * plr, uint32 id)
{
	...
	if(buffslot > 0)
	{
		...
		{
			/* apply the buff */
			SpellEntry * sp = SpellDataStorage.LookupEntry(m_buffs[buffslot]->GetInfo()->sound3);
	...
	if(((id == 3646 && plr->GetTeam() == 0) || (id == 3647 && plr->GetTeam() == 1)) && (plr->m_bgHasFlag && m_flagHolders[plr->GetTeam()] == plr->GetGUIDLow()))
	{
		...
		if(m_scores[plr->GetTeam()] == 3)
		{
			...
			SpellEntry * winner_spell = SpellDataStorage.LookupEntry(24950);
			SpellEntry * loser_spell = SpellDataStorage.LookupEntry(24951);
...
void WarsongGulch::HookFlagDrop(Player * plr, GameObject * obj)
{
	...
	SpellEntry * pSp = SpellDataStorage.LookupEntry(23333 + (plr->GetTeam() * 2));
