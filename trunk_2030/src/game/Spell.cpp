			...
			if(!IsReflected())
			{
				...
				if(p_caster && p_caster->IsInWorld() && !m_triggeredSpell)
				{
					for(i= UniqueTargets.begin();i != UniqueTargets.end();i++)
					{
						...
						p_caster->HandleProc(PROC_CAST_SPECIFIC_SPELL | PROC_CAST_SPELL,Target, m_spellInfo);
		...
		for ( i = ModeratedTargets.begin(); i != ModeratedTargets.end(); i++ )
		{
			...
			*data << (*i).TargetModType;    // uint8
			...
			if(target && target->isAlive())
				u_caster->HandleProc(PROC_RESIST_SPELL,target,m_spellInfo/*,damage*/);		/** Damage is uninitialized at this point - burlex */
		}
	}
	else
		for ( i = ModeratedTargets.begin(); i != ModeratedTargets.end(); i++ )
...
void Spell::Heal(int32 amount)
{
	...
	if(u_caster)
	{
		...
		if(critical = Rand(spellCrit))
		{
			...
			unitTarget->HandleProc(PROC_HEALED | PROC_CRIT_HEAL, u_caster, m_spellInfo, amount);
			u_caster->HandleProc(PROC_CRIT_HEAL, unitTarget, m_spellInfo, amount);
		}
	...
	if(p_caster)  
	{
		...
		p_caster->HandleProc(PROC_HEAL, unitTarget, m_spellInfo, amount);

		if(unitTarget->IsPlayer())
		{
			unitTarget->HandleProc(PROC_HEALED, p_caster, m_spellInfo, amount);
